...
const oraGateway = new MockOraGateway()
const tradeGateway = new DisabledTradeExecutionGateway()
...
app.post('/api/ora/chat', async (req, res, next) => { const parsed = chatSchema.safeParse(req.body); if (!parsed.success) return res.status(400).json({ error: 'Requisição ORA inválida.', details: parsed.error.flatten().fieldErrors }); try { res.json(await oraGateway.answer({ ...parsed.data, capability: parsed.data.capability ?? 'investor' })) } catch (error) { next(error) } })
